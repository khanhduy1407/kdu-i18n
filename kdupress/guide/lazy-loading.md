...
In short we are creating a new KduI18n instance as we normally would. Then we are creating a `loadedLanguages` array that will keep track of our loaded languages. Next is the `setI18nLanguage` function that will actually change the language in our kduI18n instance, axios and where ever else is needed.
...
Using the `loadLanguageAsync` function is straightforward. A common use case is inside a kdu-router beforeEach hook.
