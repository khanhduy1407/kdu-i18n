...
In order to implement these rules you can override the `KduI18n.prototype.getChoiceIndex` function.

Very simplified example using rules for Slavic languages (Russian, Ukrainian, etc.):
```js

const defaultImpl = KduI18n.prototype.getChoiceIndex
...
**/
KduI18n.prototype.getChoiceIndex = function (choice, choicesLength) {
  // this === KduI18n instance, so the locale property also exists here
  if (this.locale !== 'ru') {
