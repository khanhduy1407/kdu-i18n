...
If you want to use (say) `en` items when an item is not available in your desired locale, set the `fallbackLocale` option in the KduI18n constructor:
...
```console
[kdu-i18n] Value of key 'hello' is not a string!
[kdu-i18n] Fall back to translate the keypath 'hello' with 'en' locale.
```
