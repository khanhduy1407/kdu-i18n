...
import { KduDevToolsTimelineEvents } from '@kdu-i18n/kdu-devtools'
import { I18nWarnCodes, getWarnMessage } from './warnings'
import { I18nErrorCodes, createI18nError } from './errors'
import { VERSION } from './misc'

import type { ComponentInternalInstance, KNode, KNodeArrayChildren } from 'kdu'
import type { WritableComputedRef, ComputedRef } from '@kdujs/reactivity'
import type {
  Path,
  LinkedModifiers,
  PluralizationRules,
...
// extend KNode interface
declare module '@kdujs/runtime-core' {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface KNode<HostNode = RendererNode, HostElement = RendererElement> {
    toString: () => string // mark for kdu-i18n message runtime
  }
}

export const DEVTOOLS_META = '__KDU_I18N_META__'

export const TransrateKNodeSymbol = makeSymbol('__transrateKNode')
...
export const DevToolsMetaSymbol = makeSymbol('__kduI18nMeta')
export const InejctWithOption = makeSymbol('__injectWithOption')

/** @KduI18nComposition */
export type KduMessageType = string | KNode
/** @KduI18nComposition */
...
export type PreCompileHandler<Message = KduMessageType> = () => {
  messages: LocaleMessages<Message>
  functions: MessageFunctions<Message>
}

export interface CustomBlock<Message = KduMessageType> {
  locale: Locale
  resource: LocaleMessages<Message> | LocaleMessageDictionary<Message>
}

export type CustomBlocks<Message = KduMessageType> = Array<CustomBlock<Message>>
 ...
 * @KduI18nComposition
 */
export interface ComposerOptions<Message = KduMessageType> {
  /**
   * @remarks
 ...
 *
 * @remarks
 * This is the interface for being used for Kdu 3 Composition API.
 *
 * @KduI18nComposition
 */
...
> {
   ...
   *
   * @KduI18nSee [missing](composition#posttranslation)
   */
   ...
   *
   * @KduI18nSee [missing](composition#missing)
   */
...
export interface ComposerInternal {
  __transrateKNode(...args: unknown[]): KNodeArrayChildren
...
function defineCoreMissingHandler<Message = KduMessageType>(
  missing: MissingHandler
...
type GetLocaleMessagesOptions<Message = KduMessageType> = {
  messages?: LocaleMessages<Message>
  __i18n?: CustomBlocks<Message>
  flatJson?: boolean
}

export function getLocaleMessages<Message = KduMessageType>(
  locale: Locale,
...
> {
      ...
      if (
        ...
        if (__DEV__) {
          ...
          if (emitter && _fallbackRoot) {
            emitter.emit(KduDevToolsTimelineEvents.FALBACK, {
              type: warnType,
              key,
              to: 'global',
  ...
  function normalize(
    values: MessageType<string | KNode>[]
  ): MessageType<KNode>[] {
    return values.map(val =>
      isString(val) ? createKNode(Text, null, val, 0) : val
    )
  }
  const interpolate = (val: unknown): MessageType<KNode> => val as KNode
  const processor = {
    normalize,
    interpolate,
    type: 'knode'
  } as MessageProcessor<KNode>

  // transrateKNode, using for `i18n-t` component
  function transrateKNode(...args: unknown[]): KNodeArrayChildren {
    return wrapWithDeps<KNode, KNodeArrayChildren>(
      context => {
        let ret: unknown
        const _context = context as CoreTranslationContext<Messages, KNode>
        try {
          _context.processor = processor
          ret = translate<Messages, KNode>(_context, ...args)
        } finally {
      ...
      'translate',
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      root => (root as any)[TransrateKNodeSymbol](...args),
      key => [createKNode(Text, null, key, 0)],
      val => isArray(val)
    )
  }
  ...
  const composer = {
    ...
    getMissingHandler,
    setMissingHandler,
    [TransrateKNodeSymbol]: transrateKNode,
  ...
  if (__DEV__) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ;(composer as any)[EnableEmitter] = (emitter: KduDevToolsEmitter): void => {
