...
import { resolveValue } from '@kdu-i18n/message-resolver'
import { createMessageContext } from '@kdu-i18n/runtime'
import {
...
import type { CompileOptions, CompileError } from '@kdu-i18n/message-compiler'
import type {
  Locale,
  FallbackLocale,
  NamedValue,
  ...
  MessageType,
  MessageContext
} from '@kdu-i18n/runtime'
import type { AdditionalPayloads } from '@kdu-i18n/devtools-if'
import type {
  LocaleMessages,
  LocaleMessageValue,
  CoreInternalContext,
...
function resolveMessageFormat<Messages, Message>(
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    if (__DEV__ && inBrowser) {
      ...
      startTag = 'kdu-i18n-message-resolve-start'
      endTag = 'kdu-i18n-message-resolve-end'
      mark && mark(startTag)
    }
    ...
    if (__DEV__ && inBrowser) {
      ...
      if (emitter && start && format) {
        emitter.emit(KduDevToolsTimelineEvents.MESSAGE_RESOLVE, {
      ...
      if (startTag && endTag && mark && measure) {
        mark(endTag)
        measure('kdu i18n message resolve', startTag, endTag)
      }
    }
...
function compileMessageFormat<Messages, Message>(
  ...
  if (__DEV__ && inBrowser) {
    start = window.performance.now()
    startTag = 'kdu-i18n-message-compilation-start'
    endTag = 'kdu-i18n-message-compilation-end'
    mark && mark(startTag)
  }
  ...
  if (__DEV__ && inBrowser) {
    ...
    const emitter = ((context as unknown) as CoreInternalContext).__v_emitter
    if (emitter && start) {
      emitter.emit(KduDevToolsTimelineEvents.MESSAGE_COMPILATION, {
    ...
    if (startTag && endTag && mark && measure) {
      mark(endTag)
      measure('kdu i18n message compilation', startTag, endTag)
    }
  }
...
function evaluateMessage<Messages, Message>(
  ...
  if (__DEV__ && inBrowser) {
    start = window.performance.now()
    startTag = 'kdu-i18n-message-evaluation-start'
    endTag = 'kdu-i18n-message-evaluation-end'
    mark && mark(startTag)
  }
  ...
  if (__DEV__ && inBrowser) {
    ...
    const emitter = ((context as unknown) as CoreInternalContext).__v_emitter
    if (emitter && start) {
      emitter.emit(KduDevToolsTimelineEvents.MESSAGE_EVALUATION, {
    ...
    if (startTag && endTag && mark && measure) {
      mark(endTag)
      measure('kdu i18n message evaluation', startTag, endTag)
    }
  }
    ...
    onError: (err: CompileError): void => {
      ...
      if (__DEV__) {
        ...
        const emitter = ((context as unknown) as CoreInternalContext)
          .__v_emitter
        if (emitter) {
          emitter.emit(KduDevToolsTimelineEvents.COMPILE_ERROR, {
            message: source,
            error: err.message,
