...
import type { CompileOptions } from '@kdu-i18n/message-compiler'
import type {
  Locale,
  FallbackLocale,
  CoreMissingType,
...
export interface CoreInternalOptions {
  ...
  __v_emitter?: KduDevToolsEmitter // eslint-disable-line camelcase
  __meta?: MetaInfo
}
...
export interface CoreInternalContext {
  ...
  __v_emitter?: KduDevToolsEmitter // eslint-disable-line camelcase
  __meta: MetaInfo // for Kdu I18n DevTools
}

/**
 * Kdu I18n core-base version
 * @internal
 */
...
// Additional Meta for Kdu I18n DevTools
...
export function handleMissing<Message = string>(
  ...
  if (__DEV__) {
    ...
    if (emitter) {
      emitter.emit(KduDevToolsTimelineEvents.MISSING, {
        locale,
        key,
        type,
        groupId: `${type}:${key}`
