...
const outputConfigs = {
  ...
  // runtime-only builds, for '@kdu-i18n/core' and 'kdu-i18n' package only
  'esm-bundler-runtime': {
...
function createConfig(format, output, plugins = []) {
  ...
  const external =
    isGlobalBuild || isBrowserESMBuild
      ? ['kdu'] // packageOptions.enableNonBrowserBranches
  ...
  const replacements = {
    ...
    __DEV__: isBundlerESMBuild
      ...
      : // hard coded dev/prod builds
        !isProduction,
    // this is only used during Kdu's internal tests
    __TEST__: false,
    ...
    __FEATURE_FULL_INSTALL__: isBundlerESMBuild
      ? `__KDU_I18N_FULL_INSTALL__`
      : true,
    __FEATURE_LEGACY_API__: isBundlerESMBuild
      ? `__KDU_I18N_LEGACY_API__`
      : true,
    __FEATURE_PROD_KDU_DEVTOOLS__: isBundlerESMBuild
      ? `__KDU_PROD_DEVTOOLS__`
      : false,
    __FEATURE_PROD_KDU_I18N_DEVTOOLS__: isBundlerESMBuild
      ? `__KDU_I18N_PROD_DEVTOOLS__`
      : false,
